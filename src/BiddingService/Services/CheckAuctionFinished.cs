...
public class CheckAuctionFinished(ILogger<CheckAuctionFinished> logger, IServiceProvider servies)
     : BackgroundService
{
    ...
    private async Task CheckAuctions(CancellationToken stoppingToken)
    {
        ...
        if (finisedAuctions.Count == 0)
        {
            return;
        }

        logger.LogInformation("==> Found {count} auctions that have completed",
            finisedAuctions.Count);
